...
private:
  ...
  void controlLoop()
  {
    ...
    if (!sp_received_ || !odom_received_) { cmd_pub_->publish(cmd); return; }

    // Position error in world frame
    Eigen::Vector3d pos_err_w = setpoint_.pos - pos_world_;

    double norm = pos_err_w.norm();
    if (norm < tolerance_) { cmd_pub_->publish(cmd); return; }
    ...
    if (odom_in_body_frame_) vel_body_ = vel_meas_;
    else vel_body_ = q_.conjugate() * vel_meas_;  // world -> body
    // Eigen::Vector3d vel_err_b = sp_vel_b - vel_body_;

    Eigen::Vector3d ctrl_lin = kff_*sp_vel_b + kp_*pos_err_b - kd_*vel_body_;
    ctrl_lin.z() *= z_scale_;
